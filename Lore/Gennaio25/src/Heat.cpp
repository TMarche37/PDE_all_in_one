...
Heat::setup()
{
  ...
  // Create the mesh.
  {
    ...
    // Create serial mesh (generated by us).
    Triangulation<dim> mesh_serial;

    // Example: uniform mesh on [0,1] with N cells.
    const unsigned int N = 100; // 

    GridGenerator::subdivided_hyper_cube(mesh_serial,
                                        /*repetitions=*/N,
                                        /*left=*/0.0,
                                        /*right=*/1.0,
                                        /*colorize=*/true);
    // colorize=true is important:
    // in 1D it assigns boundary_id 0 to x=0 and boundary_id 1 to x=1,
...
Heat::assemble()
{
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_matrix(i, j) +=
                    ...
                    fe_values.JxW(q);
                         ...
                         //trnasportation
                  // In 1D, shape_grad(j, q) restituisce Tensor<1,1>, quindi usiamo [0] per ottenere il double
                  cell_matrix(i, j) +=
                    mu_loc *                             //
  ...
  // Homogeneous Dirichlet boundary conditions: we set the corresponding rows
  // of the matrix to identity and the corresponding entries of the right-hand
  // side to zero.
  // Dirichlet boundary conditions.
  {
    std::map<types::global_dof_index, double> boundary_values;
    Functions::ZeroFunction<dim>              bc_function;
    FunctionBoundary bc_functions;

    std::map<types::boundary_id, const Function<dim> *> boundary_functions;
    boundary_functions[0] = &bc_function;
    boundary_functions[1] = &bc_functions;
...
} // <-- chiusura corretta della funzione assemble()
