...
public:
  ...
  {
    (void)p;
    values[0] = 1.0; // componente x
    values[1] = 0.0; // componente y
    for (unsigned int i = 2; i < dim + 1; ++i)
      values[i] = 0.0; // eventuali componenti extra (es. pressione)
  }

  virtual double value(const Point<dim> &p, const unsigned int component = 0) const override
  {
    (void)p;
    if (component == 0) return 1.0;
    else return 0.0;
  }
};

class ConstantInlet : public Function<dim>
{
public:
  ConstantInlet() : Function<dim>(dim + 1) {}
  virtual void vector_value(const Point<dim> &p, Vector<double> &values) const override
  {
    (void)p;
    values[0] = 0.0; // componente x
    values[1] = 0.0; // componente y
    for (unsigned int i = 2; i < dim + 1; ++i)
      values[i] = 0.0; // eventuali componenti extra (es. pressione)
  }
...
protected:
  ...
  // Previous time step solution (owned and ghosted)
  TrilinosWrappers::MPI::BlockVector solution_old_owned;
  TrilinosWrappers::MPI::BlockVector solution_old;

  // Physical and time-stepping parameters
  double mu = 1.0;         // Dynamic viscosity (default 1.0)
  double delta_t = 0.01;   // Time step size (default 0.01)
  double theta = 1.0;      // Theta parameter for time integration (default 1.0, implicit Euler)
  ...
  // Inlet velocity.
  ConstantInlet zero_inlet_velocity;
  ...
  // (Optional) BoundaryG function/class should be declared elsewhere if used in .cpp
};
