  ...
  // Create the mesh.
  {
    ...
    pcout << "  Number of elements = " << mesh.n_global_active_cells()
          << std::endl;
            ...
            // Stampa boundary id e posizione delle facce di bordo
  pcout << "Boundary faces and their ids:" << std::endl;
  for (const auto &cell : mesh.active_cell_iterators())
    for (unsigned int f = 0; f < cell->n_faces(); ++f)
      if (cell->face(f)->at_boundary())
        pcout << "Face at boundary: id = " << cell->face(f)->boundary_id()
              << ", center = (" << cell->face(f)->center()[0] << ", "
              << cell->face(f)->center()[1] << ")" << std::endl;
  }
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
          Vector<double> f(dim + 1);
          forcing_term.vector_value(fe_values.quadrature_point(q), f);
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              ...
              if (fe->system_to_component_index(i).first < dim) // solo velocità
                  cell_rhs(i) += f[fe->system_to_component_index(i).first] *
