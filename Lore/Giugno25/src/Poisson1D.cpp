...
Poisson1D::assemble()
{
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_matrix(i, j) +=  epsilon *(1.0 + fe_values.quadrature_point(q)[0] * fe_values.quadrature_point(q)[0]) * //
                                       fe_values.shape_grad(i, q) * //
                                       fe_values.shape_grad(j, q) * //
                                       fe_values.JxW(q);
                  
                  cell_matrix(i, j) += fe_values.shape_grad(j, q)[0] * //
  ...
  // functions to
  {
    ...
    boundary_functions[1] = &dirichlet_boundary_condition;
