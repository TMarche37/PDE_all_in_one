...
DiffusionReaction::assemble()
{
  ...
  // Inizializzazione del vettore b 
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                 ...
                 cell_matrix(i, j) -= fe_values.shape_value(j, q) 
                                      * dealii::scalar_product(b, fe_values.shape_grad(i, q))
                                      * fe_values.JxW(q); // 
                }
                  ...
                  for (unsigned int q = 0; q < quadrature_boundary->size(); ++q)
                    {
                      for (unsigned int i = 0; i < dofs_per_cell; ++i)
                        {
                          FunctionBoundaryNeumann neumann_bc;
                          cell_rhs(i) +=
                            neumann_bc.value(fe_values_boundary.quadrature_point(q)) * //
  ...
  // Dirichlet boundary conditions.
  {
    ...
    FunctionBoundary bc_function_zero;

    std::map<types::boundary_id, const Function<dim> *> boundary_functions;
    boundary_functions[0] = &bc_function_zero;
    boundary_functions[2] = &bc_function_zero;
