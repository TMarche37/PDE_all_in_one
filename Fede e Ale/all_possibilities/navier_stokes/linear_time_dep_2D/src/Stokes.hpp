...
public:
  ...
  {
  public:
      DirichletFunc() : Function<dim>(dim + 1){}
      ...
      {
        if (component == 0)
          return (0.25 - p[1]*p[1]) * sin(2*M_PI*get_time());
        else
          return 0.0;
      }
...
protected:
  ...
  //viscosity coefficient [m2/s].
  const double viscosity_coefficient = 100;
  ...
  // System matrix.
  TrilinosWrappers::BlockSparseMatrix stiff_top_matrix;
  TrilinosWrappers::BlockSparseMatrix stiff_bot_matrix;
  // Mass matrix (time deriv)
  ...
  // System solution at prev time step (including ghost elements).
