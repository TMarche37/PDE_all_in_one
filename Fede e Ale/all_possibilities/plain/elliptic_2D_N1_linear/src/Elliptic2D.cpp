           ...
           ++face_number)
      {
                ...
                )
        {
          ...
          for (unsigned int q = 0; q < quadrature_boundary->size(); ++q){
            ...
            advection_coefficient.vector_value(fe_values_boundary.quadrature_point(q), b_bound);
            ...
            cout << b_bound_tensor;
            const double neumann_value = diffusion_coefficient.value(fe_values_boundary.quadrature_point(q))
                                         * scalar_product(exact_solution.gradient(fe_values_boundary.quadrature_point(q)), fe_values_boundary.normal_vector(q))
                                         -
                                         scalar_product(b_bound_tensor, fe_values_boundary.normal_vector(q))
