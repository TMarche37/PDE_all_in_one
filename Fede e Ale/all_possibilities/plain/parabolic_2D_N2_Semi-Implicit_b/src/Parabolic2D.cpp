...
Parabolic2D::assemble_rhs(const double &time)
{
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              ...
              //advection semiimplicit
              cell_rhs(i) -= scalar_product(b_tensor, solution_gradient_loc[q])
                                              * fe_values.shape_value(i,q)
                                              * fe_values.JxW(q);
            }
        }
      ...
      if (cell->at_boundary())
      {
             ...
             ++face_number)
        {
          ...
          if (cell->face(face_number)->at_boundary() && (
                  ...
                  cell->face(face_number)->boundary_id() == 122 ||
                  cell->face(face_number)->boundary_id() == 222 ||
                  cell->face(face_number)->boundary_id() == 223
                  ))
          {
            fe_face_values.reinit(cell, face_number);
  ...
  //Dirichlet boundary conditions
  {
    ...
    for (unsigned int i = 1; i < 4; ++i)
      boundary_functions[i] = &dirichlet_func;
