...

{
  ...
  {
    Parabolic2D problem(mesh, r, T, deltat, theta);
...
main(int argc, char * argv[])
{
  ...
  std::string mesh = "../mesh/mesh-square-h0.012500.msh";
  const double deltatInitial = T/3;
  const std::vector<double> deltat_vector = {deltatInitial, deltatInitial/2, deltatInitial/4, deltatInitial/8, deltatInitial/16};
  time_file_function(silent, 2,T, theta, mesh, deltat_vector, mpi_rank);
  ...
  const std::vector<double> h_vals = {1.0/5.0, 1.0/10.0, 1.0/20.0, 1.0/40.0};

  const double deltat = 0.01;

  mesh_file_function(silent, r, T, theta, deltat, meshes, h_vals,  mpi_rank);
  
  system("rm -f *.vtu *.pvtu *.vtk");
