...
public:
  ...
  public:
    ...
    value(const Point<dim> &p, const unsigned int /*component*/ = 0) const override {
      ...
      */
      const double pi = M_PI, x = p[0], y = p[1], t = get_time();
      const double cosx = cos(pi*x), sinx = sin(pi*x);
      const double cosy = cos(2.0*pi*y), siny = sin(2.0*pi*y);
      const double cost = cos(2.0*pi*t), sint = sin(2.0*pi*t);
      
      const double u =  sint*sinx*siny;
      
      const double du_dx = pi*sint*cosx*siny;
      const double du_dy = 2.0*pi*sint*sinx*cosy;
      const double du_dt = 2.0*pi*cost*sinx*siny;
      
      const double d2dx2 = -pi*pi*u;
      const double d2dy2 = -4.0*pi*pi*u;
  ...
  public:
          ...
          const unsigned int /*component*/ = 0) const override
    {
      ...
      double cosy = cos(2.0*pi*y), siny = sin(2.0*pi*y);
      double cost = cos(2.0*pi*t), sint = sin(2.0*pi*t);
      
      return sint*sinx*siny;
             ...
             const unsigned int /*component*/ = 0) const override
    {
      ...
      // duex / dx
      result[0] = pi*sint*cosx*siny;
      
      // duex / dy
      result[1] = 2.0*pi*sint*sinx*cosy;
