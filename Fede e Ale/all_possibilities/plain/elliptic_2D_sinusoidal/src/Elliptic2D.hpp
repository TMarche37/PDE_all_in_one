...
public:
  ...
  public:
          ...
          const unsigned int /*component*/ = 0) const override
    {
      ...
      return -mu *
         (
            sin(2*M_PI*p[1]) * p[1]*p[1]  *
            (
              -4*M_PI*M_PI * sin(2*M_PI*p[0]) * p[0]*p[0]
              + 8*M_PI * cos(2*M_PI*p[0]) * p[0]
              + 2 * sin(2*M_PI*p[0])
            )
            +
            sin(2*M_PI*p[0]) * p[0]*p[0] *
            (
              -4*M_PI*M_PI * sin(2*M_PI*p[1]) * p[1]*p[1]
              + 8*M_PI * cos(2*M_PI*p[1]) * p[1]
              + 2 * sin(2*M_PI*p[1])
            )
         )
  ...
  public:
    ...
    value(const Point<dim> &p,
          const unsigned int /*component*/ = 0) const override
    {
      return sin(2*M_PI*p[0])*sin(2*M_PI*p[1]) * p[0]*p[0] * p[1]*p[1] + offset;
    }
             ...
             const unsigned int /*component*/ = 0) const override
    {
      ...
      result[0] = sin(2*M_PI*p[1]) * p[1]*p[1] *
                  (
                    2*M_PI * cos(2*M_PI*p[0]) * p[0]*p[0] 
                    + 
                    sin(2*M_PI*p[0]) * 2*p[0] 
                  );

      result[1] = sin(2*M_PI*p[0]) * p[0]*p[0] *
                  (
                    2*M_PI * cos(2*M_PI*p[1]) * p[1]*p[1] 
                    + 
                    sin(2*M_PI*p[1]) * 2*p[1] 
                  );
