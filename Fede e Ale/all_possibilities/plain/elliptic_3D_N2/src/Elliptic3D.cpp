    ...
    for (unsigned int q = 0; q < n_q; ++q)
    {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
        {
          ...
          //advection term
          cell_matrix(i,j) += scalar_product(b_tensor, fe_values.shape_grad(j, q))
      ...
      for (unsigned int face_number = 0; face_number < cell->n_faces(); ++face_number)
      {
            ...
            (cell->face(face_number)->boundary_id() == 1))
        {
          ...
          for (unsigned int q = 0; q < quadrature_boundary->size(); ++q){
            ...
            const double neumann_value = neumann_func.value(fe_values_boundary.quadrature_point(q));
...
double
Elliptic3D::compute_error(const VectorTools::NormType &norm_type) const
{
