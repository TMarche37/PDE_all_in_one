...
Parabolic1D::assemble_matrices()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    for (unsigned int q = 0; q < n_q; ++q)
    {
      ...
      Tensor<1,dim> advection_coefficient_loc;
      advection_coefficient_loc[0] = advection_coefficient.value(fe_values.quadrature_point(q));
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
        {
          ...
          //advection
          cell_stiffness_matrix(i,j) += scalar_product(advection_coefficient_loc, fe_values.shape_grad(j,q))
