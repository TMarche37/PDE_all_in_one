...
ADR2D::assemble()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    for (unsigned int q = 0; q < n_q; ++q)
    {
      ...
      {
        transport_vector[i] = transport_coefficient.value(fe_values.quadrature_point(q), i);
      }


      //cout << b_tensor << std::endl;
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
        {
          ...
          //advection term
          cell_matrix(i,j) += scalar_product(transport_vector, fe_values.shape_grad(i, q))*
        ...
        //cout << i << "  " <<  forcing_term.value(fe_values.quadrature_point(q)) << std::endl;
      }
    }
           ...
           ++face_number)
      {
             ...
             cell->face(face_number)->boundary_id() == 3))
        {
          ...
          for (unsigned int q = 0; q < quadrature_boundary->size(); ++q)
            for (unsigned int i = 0; i < dofs_per_cell; ++i) {
              cell_rhs(i) +=
                      function_Phi.value(fe_values_boundary.quadrature_point(q)) * // phi(xq)
                      fe_values_boundary.shape_value(i, q) *      // v(xq)
                      fe_values_boundary.JxW(q);  // Jq wq             // Jq wq


              //cout << i << "  " << fe_values_boundary.JxW(q) << std::endl;
            }
        }
      }
    }
...
double
ADR2D::compute_error(const VectorTools::NormType &norm_type) const
{
