...
public:
    ...
    public:
      ...
      // Evaluation.
        ...
        {
          return (
                  (M_PI/2.0)*sin(M_PI/2.0*p[0])*cos(M_PI/2.0*get_time()) +
                  ((pow(M_PI, 2.0))/4.0 - 1.0)*sin(M_PI/2.0*p[0])*sin(M_PI/2.0*get_time())+
                  (M_PI/2.0)*cos(M_PI/2.0*p[0])*sin(M_PI/2.0*get_time())
          );
        }
    };
    ...
    //Forcing term.
    class ZeroFucntion : public Function<dim>
    {
    public:
        // Constructor.
        ZeroFucntion()
    ...
    public:
        ...
        value(const Point<dim> &p,
              const unsigned int /*component*/ = 0) const override
        {
          return sin(M_PI/2.0*p[0])*sin(M_PI/2.0*get_time());
        ...
        {
          Tensor<1, dim> result;
          result[0] = (M_PI/2.0)*cos(M_PI/2.0*p[0])*sin(M_PI/2.0*get_time());
    ...
    // Constructor.
    Poisson1D(const unsigned int &N_,
