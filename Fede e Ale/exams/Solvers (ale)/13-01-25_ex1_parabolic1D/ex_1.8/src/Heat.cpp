      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
          ...
          Tensor<1, dim> b_loc;
          b_loc[0] = b.value(fe_values.quadrature_point(q), 0); 
          // b_loc[1] = b.value(fe_values.quadrature_point(q), 1);
          const double reaction_loc = reaction_term.value(fe_values.quadrature_point(q));
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_stiffness_matrix(i, j) +=
                    ...
                    +
                    scalar_product(b_loc, fe_values.shape_grad(j,q))
                    * fe_values.shape_value(i, q) * fe_values.JxW(q)
                    +
                    reaction_loc * fe_values.shape_value(i,q) 
                    * fe_values.shape_value(j,q) * fe_values.JxW(q);

                  // Stabilization term
                  double h = 1.0/N;
                  double th = h * b_loc[0] / 2.0;
                  double tau = h/(2*b_loc[0]) * (1.0/tanh(th) - 1.0/th);

                  cell_mass_matrix(i,j) +=
                  tau * fe_values.shape_value(j,q) / deltat
                  * scalar_product(b_loc, fe_values.shape_grad(i,q))
                  * fe_values.JxW(q);

                  cell_stiffness_matrix(i, j) +=
                    tau * b_loc[0] * fe_values.shape_grad(j,q)
                    * b_loc[0] * fe_values.shape_grad(i,q) * fe_values.JxW(q);
...
Heat::assemble_rhs(const double &time)
{
  ...
  FEValuesExtractors::Scalar val(0);
...
Heat::solve_time_step()
{
  ...
  pcout << "  " << solver_control.last_step() << " BiCGSTAB iterations" << std::endl;
