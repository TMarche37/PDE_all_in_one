...
public:
  ...
  static constexpr double diff = 8;
  static constexpr double transp = 0.5;
  static constexpr double react = 2.0;
  ...
  public:
    ...
    value(const Point<dim> &p,
          const unsigned int /*component*/ = 0) const override
    {
      return (-2 - diff*4 + transp*2 + react)*exp(2.0*(p[0] - get_time()));
    }
  };
  ...
  public:
    ...
    value(const Point<dim> &p,
          const unsigned int /*component*/ = 0) const override
    {
      return diff * 2 * exp(2*p[0]-2*get_time());
    }
  };
...
protected:
  ...
  FunctionDirichlet dirichlet_function;
  FunctionNeumann neumann_function;
