...
void NavierStokes::setup()
{
  ...
  // -----------------------------
  {
    ...
    // reorder: velocity first, then pressure (same as Stokes)
...
void NavierStokes::assemble()
{
  ...
  // Need old velocity at quadrature points for RHS and convection.
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    {
      const Tensor<1, dim> u_adv = u_old_q[q];
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        ...
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
        {
          ...
          // (u^{n-1} dot grad u^n, v)  (Picard linearization)
          const Tensor<1, dim> convection = grad_j_u * u_adv;
          cell_matrix(i,j) += (convection * phi_i_u) * fe_values.JxW(q);

          // - (p^n, div v)
          cell_matrix(i,j) -= div_i_u * phi_j_p * fe_values.JxW(q);

          // - (q, div u^n)  (same sign convention as Stokes)
    ...
    // Boundary term: -int phi dot v = +int g (n dot v)  since phi = -g n
...
void NavierStokes::output() const
{
  ...
  // parallel partition (like Stokes)
  ...
  const std::string basename = "output-navier-td";
...
void NavierStokes::run()
{
  // Heat-style initialization block
  {
    ...
    // Set initial conditions (u0 = 0, p0 = 0)
    solution_owned = 0.0;
    solution       = solution_owned;
