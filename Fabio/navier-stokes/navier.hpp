...
/**
 * Time-dependent Navier-Stokes (Backward Euler + Picard linearization):
 *
 *   (u^n - u^{n-1})/dt + (u^{n-1} dot grad) u^n - mu*Laplace(u^n) + grad p^n = 0
 *   div u^n = 0
 *   (du/dn - p n) = phi   on boundary
 */
class NavierStokes
{
public:
  ...
  // g(x) used in phi = -g n. Same convention as Stokes.
  class BoundaryG
  {
  public:
  ...
  // Triangular block preconditioner (same as Stokes).
  ...
  // Constructor: keep Stokes-style args (mesh, degrees, dt, T).
