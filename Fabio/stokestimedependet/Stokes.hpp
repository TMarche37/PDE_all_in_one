...
/**
 * Time-dependent Stokes (Backward Euler) with Neumann stress boundary condition:
 *
 *   (u^n - u^{n-1})/dt - mu*Δu^n + ∇p^n = 0
 *   div u^n = 0
 *   (∂u/∂n - p n) = φ   on ∂Ω
 *
 * Exam data: u0 = 0,  φ = -g(x) n,  g depends on boundary_id.
 */
class Stokes
{
public:
  ...
  // g(x) used in φ = -g n. In the exam it is piecewise constant per boundary id.
  class BoundaryG
  {
  public:
  ...
  // Triangular block preconditioner (same as your Stokes).
  ...
  // Heat-style driver (setup + time loop)
  void run();
...
protected:
  ...
  // Constraints needed to fix pressure nullspace (pure Neumann)
  ...
  // Old solution (u^{n-1}, p^{n-1}) like Heat uses solution as "old"
