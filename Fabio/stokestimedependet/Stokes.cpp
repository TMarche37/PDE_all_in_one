...
void Stokes::setup()
{
  ...
  /*
  // ------------------------------------------------------------
  // 4) Constraints: enforce zero-mean pressure  ∫_Ω p dx = 0
  // (manual implementation, no DoFTools::make_zero_mean_constraints)
  // ------------------------------------------------------------
  {
    constraints.clear();
    constraints.reinit(locally_relevant_dofs);

    // 1) Costruisco l'IndexSet dei DoF di pressione
    ComponentMask pressure_mask(dim + 1, false);
    pressure_mask.set(dim, true);
    const IndexSet pressure_dofs = DoFTools::extract_dofs(dof_handler, pressure_mask);

    // 2) Calcolo i pesi w_i = ∫ phi_i^p dx  (solo DoF pressione)
    TrilinosWrappers::MPI::Vector w;
    w.reinit(block_owned_dofs[1], MPI_COMM_WORLD); // block 1 = pressure indices [n_u, n_u+n_p)
    w = 0.0;
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int q = 0; q < quadrature->size(); ++q)
        ...
        {
          // i-esimo shape: che componente è?
          const unsigned int comp = fe->system_to_component_index(i).first;
          if (comp != dim) // non è pressione
            continue;

          const types::global_dof_index gi = dof_indices[i]; // global index in [n_u, n_u+n_p)
          w[gi] += fe_values.shape_value(i, q) * fe_values.JxW(q);
        }
    }

    w.compress(VectorOperation::add);

    // 3) Scelgo un pivot k con w_k != 0 (deterministico: il minimo indice globale)
    types::global_dof_index local_candidate = numbers::invalid_dof_index;
    ...
    for (auto it = pressure_dofs.begin(); it != pressure_dofs.end(); ++it)
    {
      ...
      const double wi = w[gi];
      if (std::abs(wi) > 1e-14)
      {
        local_candidate = gi;
        break; // primo localmente-owned con peso non nullo
      }
    }

    // Riduzione MPI: scelgo il pivot globale minimo fra i candidati
    // Se un rank non ha candidato, usa un valore "grande".
    const types::global_dof_index big =
      std::numeric_limits<types::global_dof_index>::max();

    const types::global_dof_index local_val =
      (local_candidate == numbers::invalid_dof_index) ? big : local_candidate;

    const types::global_dof_index pivot =
      Utilities::MPI::min(local_val, MPI_COMM_WORLD);

    AssertThrow(pivot != big, ExcMessage("Could not find a pressure DoF with nonzero integral weight."));

    // 4) Impongo: sum_i w_i p_i = 0  =>  p_pivot = - sum_{i!=pivot} (w_i/w_pivot) p_i
    //    Creo la riga di vincolo SOLO sul rank che possiede 'pivot'.
    if (locally_owned_dofs.is_element(pivot))
    {
      const double w_pivot = w[pivot];
      AssertThrow(std::abs(w_pivot) > 1e-14, ExcMessage("Pivot weight is zero (unexpected)."));

      constraints.add_line(pivot);
      constraints.set_inhomogeneity(pivot, 0.0);
      ...
      for (auto it = pressure_dofs.begin(); it != pressure_dofs.end(); ++it)
      {
        ...
        const double wi = w[gi]; // ok anche se non owned: w ha indici globali
        if (std::abs(wi) < 1e-14)
          continue;

        constraints.add_entry(pivot, gi, -wi / w_pivot);
      }
    }
    ...
    if (pcout.is_active())
      pcout << "Zero-mean pressure constraint added with pivot DoF = " << pivot << std::endl;
  }
  */
...
void Stokes::assemble()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    for (unsigned int q = 0; q < n_q; ++q)
    {
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        ...
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
        {
          ...
          // - (q, div u^n)  (same sign convention as your Stokes stazionario)
    ...
    // Boundary term: -∫ phi·v = +∫ g (n·v)  since phi = -g n
    for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
    {
      ...
      const types::boundary_id bid = cell->face(face)->boundary_id();
      const double g = g_data.value_on_boundary_id(bid);

      for (unsigned int qf = 0; qf < n_q_face; ++qf)
      {
        const Tensor<1,dim> n = fe_face_values.normal_vector(qf);

        for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
          const Tensor<1,dim> v_i = fe_face_values[velocity].value(i, qf);
          cell_rhs(i) += g * (n * v_i) * fe_face_values.JxW(qf);
        }
      }
    }
