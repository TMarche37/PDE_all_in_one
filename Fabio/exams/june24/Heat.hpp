...
public:
  ...
  class Functionalfa : public Function<dim>
  {
  public:
    // Constructor.
    Functionalfa()
    ...
    value(const Point<dim> &p,
          const unsigned int /*component*/ = 0) const override
    {
      return exp(-2.0*this->get_time());
    }
  };

  // Constructor.
  Heat(const unsigned int                              &N_el_,
       const unsigned int                              &r_,
       const double                                    &T_,
       const double                                    &theta_,
       const double                                    &delta_t_,
       const std::function<double(const Point<dim> &)> &k_,
...
protected:
  ...
  std::function<double(const Point<dim> &)> k;
