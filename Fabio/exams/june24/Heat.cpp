...
Heat::assemble()
{
  ...
  // (faces).
  QGaussSimplex<dim-1> face_quadrature(r+1);
      ...
      for (unsigned int q = 0; q < n_q; ++q)
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              ...
              // Transport.
              cell_rhs(i) -= (1.0 - theta) * solution_old_values[q] * //
                             fe_values.shape_value(i, q) *  //
                             fe_values.JxW(q);
        ...
        for (unsigned int face=0; face<GeometryInfo<dim>::faces_per_cell; ++face)
          if (cell->face(face)->at_boundary() &&
              cell->face(face)->boundary_id() == 1) // x=1
          {
            fe_values_boundary.reinit(cell, face);

            const double beta = 2.0 * std::exp(2.0 - 2.0*time);

            for (unsigned int q=0; q<fe_values_boundary.n_quadrature_points; ++q)
              for (unsigned int i=0; i<dofs_per_cell; ++i)
                cell_rhs(i) += beta * fe_values_boundary.shape_value(i,q) * fe_values_boundary.JxW(q);
          }
