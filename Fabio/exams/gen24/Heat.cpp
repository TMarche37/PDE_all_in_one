...
Heat::assemble()
{
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_matrix(i, j) +=
                    ...
                    fe_values.JxW(q);
                  // Transport.
                  cell_matrix(i, j) += theta * scalar_product(b_loc, fe_values.shape_grad(j, q)) * //
                                       fe_values.shape_value(i, q) * //
                                       fe_values.JxW(q);
                }
              ...
              // Transport.
              cell_rhs(i) -= (1.0 - theta) * scalar_product(b_loc, solution_old_grads[q]) * //
                             fe_values.shape_value(i, q) *  //
                             fe_values.JxW(q);
  ...
  // Dirichlet boundary conditions.
  {
    ...
    boundary_functions[4] = &bc_function;
